###########################################################################################################
# Visualize Data 
# Plot a batch of OBGL-DDI
# Code adopted from SEAL_OBG repo (Many code overlap)
# https://github.com/facebookresearch/SEAL_OGB/blob/master/seal_link_pred.py
###########################################################################################################
...
from torch_geometric.utils import to_networkx, to_undirected, add_self_loops, negative_sampling
...
class SEALDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        if 'edge_weight' in self.data:
            ...
            edge_weight = np.ones(self.data.edge_index.size(1)) ### torch.ones(self.data.edge_index.size(1), dtype=int)
        A = ssp.csr_matrix(
...
def visualize(G, hd_color_map, graph_index, seed=123):
  f = plt.figure(figsize=(20, 20))
  limits = plt.axis('off')

  nodes = nx.draw_networkx_nodes(G, pos=nx.spring_layout(G, seed=seed), \
                                 label=None, node_color=hd_color_map, node_shape='o', node_size=150)
  edges = nx.draw_networkx_edges(G, pos=nx.spring_layout(G, seed=seed), alpha=0.5)
  
  if color_map is not None:
    plt.scatter([],[], c='yellow', label='Nodes with label 0', edgecolors="black", s=140)
    plt.scatter([],[], c='#c0affb', label='Nodes with label 1', edgecolors="black", s=140)
    plt.scatter([],[], c='#e6a176', label='Nodes with label 2', edgecolors="black", s=140)
    plt.scatter([],[], c='#00678a', label='Nodes with label 3', edgecolors="black", s=140)
    plt.scatter([],[], c='#984464', label='Nodes with label 4', edgecolors="black", s=140)
    plt.scatter([],[], c='#6AA84F', label='Nodes with label 5', edgecolors="black", s=140)
    plt.scatter([],[], c='red', label='Nodes with label 6', edgecolors="black", s=140)
    plt.legend(prop={'size': 13}, handletextpad=0)
  nodes.set_edgecolor('black')
  
  plt.show()
  
  f.savefig('ddi_node_c_%d.png' % graph_index)

def myto_networkx(data, node_attrs=None, edge_attrs=None, to_undirected=False,
    ...
    node_summary = Counter(values['z'])

    return {ni: values['z'][ni] for ni in range(len(G.nodes))}, node_summary
...
for gindex, g in enumerate(loader):

    g = g.to(device)
    
    node_size = 100
    with_labels = True
    G = to_networkx(g, node_attrs=['z'], to_undirected=True)

    if (gindex%10 == 0):
    	print("Processing %dth graph" % gindex)
    

    node_label, node_summary = myto_networkx(g, node_attrs=['z'], to_undirected=True)

    # novels = node_label.difference(indexSet)
    # print(len(novels))
    # indexSet = indexSet.union(novels)
    ...
    cmap = [color_map[cii] for cii in node_label.values()]

    for node in node_summary:
    	node_prop[node] += node_summary[node]


    if 6 in node_summary:
	    print("Plotting for batch with %d nodes" % len(G))
	    visualize(G, cmap, gindex)
    
print("Out of total %d nodes" % str(sum(node_prop.values())))
for node in node_prop:
	print("\tNode %d: %d" % (node, node_prop[node]))
